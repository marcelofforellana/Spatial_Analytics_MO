...
dataINE<-data.table(readRDS("Tabla_19_19.rds"))
```


```{r cargando base de datos}
str(dataINE) #Con este coando podemos ver que la base de datos tiene 929527 observaciones de 28 variables
summary(dataINE) # AQUI NOS ARROJA UNA PEQUEÑA TABLA DESCRIPTIVA CON ALGUNOS ESTADISTICOS Y DATOS RELEVANTES, A PARTIR DE ESTO NOS PODEMOS DEDUCIR QUE LA VARIABLE B6 TIENE MAS CANTIDAD DE DATOS NA CON 908144

#dataINE[rowSums(is.na(al_29_03_2020))>0,]

dataINE[,.N,by=.(b6)]

```
```{r Pregunta 2}
P2<- dataINE[,sum(V1,na.rm=T),by=.(region,ano_trimestre,mes_central)] #Creamos el objeto P2 
#P2
#summary(P2) #observamos descripcion de la base de datos P2
#str(P2)
P2_last_year<- P2[ano_trimestre==2020] # 202 es el último mes del año y tiene un sólo mes en la base de datos, lo ocupamos para crear la variable P2_last_year
P2_last_year

#P2_last_month
ggplot(data = P2_last_year, aes(x = region, y = V1)) + 
  geom_bar(stat="identity",aes(fill= as.factor(region))) +
  scale_y_continuous(labels = number_format(scale = 0.000001, suffix = "M", big.mark = ".", decimal.mark = ",")) + 
  labs(x = "Región por número", y = "Suma total de personas en el último mes y año", fill = "Color")

ggplot(data = P2_last_year, aes(x = as.factor(region), y = V1)) + 
  geom_bar(stat="identity",aes(fill= as.factor(region))) +
  scale_y_continuous(labels = number_format(scale = 1, big.mark = ".", decimal.mark = ",")) + 
  labs(x = "Región por número", y = "Suma total de personas en el último mes y año", fill = "Regiones")
```
```{r Pregunta 3}
#Creación del objeti P3
#P3<-dataINE[(region==1,mes_central,ano_trimestre,cae_general_red)]
P3<- dataINE[region==10,.N,,by=.(cae_general_red,ano_trimestre,mes_central)]
P3
```


```{r Pregunta 3}
ggplot(data= P3,mapping = aes(x=mes_central,y=N))+geom_line(position=position_identity())+facet_wrap(~cae_general_red)+geom_smooth(method = 'lm',se=F) + geom_smooth(method = 'loess',col='red',se=F)+labs(x = "Meses", y = "Cantidad de casos", fill = NULL)


```
```{r Pregunta 4}
#Usamos el codigo anterior para crear una nueva variable
P4<- dataINE[region==10,.N,,by=.(cae_general_red,ano_trimestre,mes_central,r_p_c)]
#Cantidad de cesantes
cesantes_region_ultimo_mes<- dataINE[ano_trimestre==2020 & region==10 & cae_general_red=="Cesantes",.N,,by=.(cae_general_red,ano_trimestre,mes_central,r_p_c)]
#cesantes_region_ultimo_mes

total_cesantes<-dataINE[ano_trimestre==2020 & region==10 & cae_general_red=="Cesantes",.N,by=.(cae_general_red,ano_trimestre,mes_central,r_p_c)]
total_cesantes

sum(total_cesantes[,N]) #suma total de los cesantes de la tercera región correspondiente al  mes 12 y año 2020

```
```{r Pregunta 4b}
max_cesantes<- as.numeric(max(total_cesantes[,N])) #verificamos cuanto es el máximo

comuna_max_cesantes<-total_cesantes[N==max_cesantes]
comuna_max_cesantes #comuna con más cesantes
```
```{r Pregunta 4c}
#ru = Desempleados/Ocupados + Desempleados


total_ocupados<-dataINE[ano_trimestre==2020 & region==10 & cae_general_red=="Ocupados",.N,by=.(cae_general_red,ano_trimestre,mes_central,r_p_c)]
sum(total_ocupados[,N])
tasa_desempleo<- (sum(total_cesantes[,N])/(sum(total_cesantes[,N])+sum(total_ocupados[,N])))
tasa_desempleo
```
...
#Calcule la tasa de desempleo para cada comuna de la región utilizando la misma fórmula. ¿Cuál es la comuna con mayor tasa de desempleo? ¿Es la misma comuna con mayor número de desempleados? Si no, por qué es esto? Guarde estos resultados en un objeto P5 que contenga una fila con el código comunal, otra fila con el número de desempleados, y una tercera fila con la tasa de desempleo por cada comuna.

P5<-merge(total_cesantes,total_ocupados,by="r_p_c")
P5$ano_trimestre.x<-NULL
P5$ano_trimestre.y<-NULL
P5$mes_central.x<-NULL
P5$mes_central.y<-NULL
colnames(P5)[3] <- "cant_cesantes"
colnames(P5)[5] <- "cant_ocupados"
P5$cae_general_red.x<-NULL
P5$cae_general_red.y<-NULL

P5_aux<-P5[,tasa_de_desempleo:=(cant_cesantes/(cant_cesantes+cant_ocupados))]
P5_aux

max_t_desempleo<- (max(P5_aux[,tasa_de_desempleo])) #verificamos cuanto es el máximo
max_t_desempleo

comuna_max_tdesempleo<-P5_aux[tasa_de_desempleo==max_t_desempleo]
comuna_max_tdesempleo #comuna con máxima tasa de desempleo

ggplot(data=P5_aux,mapping = aes(x=reorder(r_p_c,tasa_de_desempleo),y=tasa_de_desempleo))+
  geom_bar(stat = "identity",aes(fill= as.factor(r_p_c)))+
  coord_flip()+
  scale_y_continuous(label=number_format(scale = 1, decimal.mark = ","))+
  labs(x= "Comunas",y="Tasa de desempleo",title = "Tasa de desempleo por comuna de la region 10", fill="Comunas")
...
paleta <- rev(brewer.pal(n = 3,name = "Blues"))

mapa_comunas_dt<-data.table(mapa_comunas)
mapa_comuna_r10<- mapa_comunas_dt[codigo_region==10]

colnames(mapa_comuna_r10)[1] <- "r_p_c"
mapa_comuna_r10[,"r_p_c"]<-lapply(mapa_comuna_r10[,"r_p_c"],as.double)

mapa_cesantes_comuna_r10 <- merge(P5_aux,mapa_comuna_r10,by="r_p_c")

mapa_cesantes_comuna_r10

ggplot(mapa_cesantes_comuna_r10)+
  geom_sf(aes(fill=cant_cesantes,geometry=geometry),colour="black")+
  scale_fill_gradientn(colours=rev(paleta))+ggtitle("Cantidad de cesantes por comuna")+
  theme(plot.title = element_text(size=rel(1.5),vjust = 2))+
  labs(fill="Cantidad de personas cesantes")
...
#En base al objeto P5 creado en la pregunta anterior, responda:
#• Haga un mapa comunal coloreando por la tasa de desempleados. Haga este mapa de tipo estático uti- lizando la función geom_sf del paquete ggplot2 y los mapas que vienen dentro del paquete chilemapas. Asegurese de explicar:
#– Su elección de categorización de los colores y su elección de paleta de colores.
#• ¿Qué puede decir de la distribución espacial del desempleo?

ggplot(mapa_cesantes_comuna_r10)+
  geom_sf(aes(fill=tasa_de_desempleo,geometry=geometry),colour="black")+
  scale_fill_gradientn(colours=rev(paleta))+ggtitle("Porcentaje de cesantía por comuna")+
  theme(plot.title = element_text(size=rel(1.5),vjust = 2))+
  labs(fill="Tasa de desempleo")
