...
casos[,.N,by=.(Región)] #cantidada de casos por región

names(casos)

casos<-casos[Región=="Metropolitana",] #toma todas las filas que tienen Region==Metropolitana
casos
saveRDS(casos,"Class_03/casosRM.rds")

write.csv(casos,file = 'Class_03/CasosCovid_RM.csv',fileEncoding = 'UTF-8')

writexl::write_xlsx(casos,path = "Class_03/CasosenExcel.xlsx") #los dos puntos significa llamar una funcion del paquete
...
casosRM[`Centro de salud`=="Clínica Alemana",`Centro de salud`:="Clinica Alemana"] #corrige y uno a las clinicas alemanas con tilde y sin tilde
...
obj1[,porc:=N/sum(N,na.rm = T)] #porcentaja de casos por clínica con respecto al total

# collapsing (colapsar) by average age


A<-casosRM[,.(AvAge=mean(Edad,na.rm = T)),by=.(`Centro de salud`)] #TABLA CON MEDIA DE EDAD POR CENTRO DE SALUD
A

B<-casosRM[,.(Total_centro=.N),by=.(`Centro de salud`)] #suma de casos confirmados por centro de salud
B
C<-casosRM[Sexo=="Femenino",.(Total_Centro_Mujeres=.N),by=.(`Centro de salud`)] #cantidad de mujeres por centro de salud

D<-casosRM[Sexo=="Masculino",.(Total_Centro_Hombres=.N),by=.(`Centro de salud`)] #cantidad de hombres que hay por centro de salud
